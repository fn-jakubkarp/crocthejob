...
For every candidate:
- Skip if the URL or company+title combo already exists in `data/jobs.json`
- Skip if the company+role is already applied to (see Step 0.2)

### Step 2.25: Keep the Posting Text

Every posting a `detail` or `WebFetch` call returned in full gets written to
`documents/postings/<company-slug>-<role-slug>.md`, and the entry records that path in
`posting_file` (Step 4). Listings expire; this is the copy that survives, and the board
reads it there. If the filename is taken by a different posting, append `-2`, `-3`.

Write the text the fetch returned, never a summary and never a reconstruction. A posting
that could not be fetched in full gets no file and no `posting_file`.

```markdown
# <Title> - <Company>

<location> · <work mode> · <salary or "no salary stated">
Source: <url>
Saved: YYYY-MM-DD from <portal>
...
Save nothing for a posting fetched only as a search-result snippet: half a posting under
a filename that promises the whole one is worse than the link alone.
...
1. Add ALL fetched jobs (new and skipped) to `data/jobs.json` with structure:
```json
{
  "seen": {
    "<url_or_company_title_key>": {
      ...
      "portal": "<source portal skill, e.g. justjoinit-search>",
      "posting_file": "documents/postings/<company-slug>-<role-slug>.md"
    }
  }
}
```

`posting_file` is the Step 2.25 copy, repo-relative. Absent means nothing was saved -
the board's only existence check, so never write the field without writing the file.
...
The kanban board (`studio/apps/web/`) writes two fields of its own on entries the user moves by hand: `status`, and `notes` for anything they typed about the posting. One status is the board's alone - **`dismissed`**, meaning the user read the posting and ruled it out themselves, with the reason in `notes`. Treat those entries as closed: they stay in the dedup set like any other seen entry, `/rank` never re-scores them, and nothing here overwrites their `notes`. `status` in the schema above is therefore one of `new/skipped/evaluated/ranked/expired/dismissed`, plus the application stages the board sets (`applied/screening/tech_interview/offer/rejected`).

**`ghosted` and `withdrawn` are no longer statuses.** Every way an application ends without an offer is now `status: "rejected"`, and *which* way is in the `outcome` array. Writing either old value still works - the board translates it on read and on write - but write `rejected` plus the tag instead. There are only ten statuses now, not twelve.

**Six more fields belong to the board alone. Carry them through untouched and never write them here:**

| Field | Where it lives | What it means |
|---|---|---|
| `next_id` | file root, not an entry | The next id to hand out. Leave it alone; if it goes missing the board recomputes it from the highest id present. |
| `id` | integer, first key of an entry | The role's stable number, the handle the user refers to a posting by (`#153`). Assigned once by the board, never reused, never renumbered. **Do not invent one for a new entry** - write the entry without an `id` and the board backfills it on next load. |
| `status_date` | ISO date | The day `status` last changed, which is how the board ages a live application. |
| `applied_date` | ISO date | The day the application actually went out, set by hand on the board. Separate from `status_date`, which is only ever the day of the change: an application logged after the fact needs to say when it happened. |
| `duplicate_of` | key, or `null` | The entry this one is a copy of, so the board can hide it. Explicit `null` means the user checked and ruled it standalone - absent and `null` are **not** the same, so never replace one with the other. |
| `outcome` | array of tags, next to `status` | Why a `rejected` application ended, and combinable: `ghosted`, `withdrawn`, `on_hold`, `failed_screening`, `failed_tech`, `failed_behavioural`. `["ghosted", "failed_tech"]` is a real entry - they went quiet after the technical. **Absent or empty means they simply said no**; there is deliberately no tag for that, so never add one. |

2. Only present jobs NOT already in the seen list or tracker.

3. **Compare URLs in their canonical form when deduplicating.** Portals hand the same posting back under tracking parameters and locale prefixes, and treating those as distinct is what produced ~7 twin entries in this file: `nofluffjobs.com/pl/job/qa-test-engineer-merit-remote` and `nofluffjobs.com/job/qa-test-engineer-merit-remote?utm_source=freehire.me` are one Merit listing. Before comparing a fetched URL against `data/jobs.json`, lowercase the host, drop a leading `www.`, drop a leading language segment (`/pl`, `/en`, …), strip a trailing slash, and remove `utm_*` and other referrer parameters (`ref`, `source`, `src`, `fbclid`, `gclid`, `trk*`). `packages/jobs-data/dupes.ts` holds the same rule in code if you need the exact list. A match means the posting is already seen - do not add a second entry.
...
If the user picks a number, invoke the **job-application-assistant** skill workflow (fit evaluation first, then the tailored CV if approved).

If the run found many new jobs (roughly 8+), also suggest `/rank` - it batch-scores all new postings against the full fit framework and returns a ranked shortlist, which beats eyeballing a long table. (`/rank` sets the `ranked` and `expired` status values in `data/jobs.json`; treat both as already-seen for dedup purposes.)

### Step 6: Record an Application (Optional)

If the user decides to apply to any job, set that entry's `status` to `applied` and its
`applied_date` in `data/jobs.json`. A posting they applied to that this run did not find
gets a new entry keyed `manual:<company-slug>:<role-slug>` with `portal: "manual (user)"`,
the same shape the board's **Add posting** dialog writes.
...
2. **Respect deduplication.** Always check `data/jobs.json` before presenting - it holds every posting ever seen and every application ever made.
