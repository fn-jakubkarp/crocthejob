...
- **`/upskill`** — aggregate mode: analyses all scored jobs in `data/jobs.json`
...
### Aggregate mode
1. Read `data/jobs.json`. Take every entry `/rank` has scored, i.e. every one carrying a `rank_score`.
2. For each, note `title`, `company` and `rank_score`. `rank_score` is a 0–100 score where 100 = perfect fit. You will use it to weight gaps — a lower score means the role exposed more gaps. Where `rank_dimensions` is present it splits that score into `technical`, `experience`, `behavioral` and `career`, so a gap can be attributed rather than guessed at.
...
4. No data from `data/jobs.json` is used in targeted mode.
...
3. **Targeted mode ignores the collection.** In targeted mode, analyse only the fetched posting. Do not load or reference `data/jobs.json`.
