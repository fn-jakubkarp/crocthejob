...
You are running the onboarding setup for the Croc the Job framework. Your goal is to collect the user's professional information and populate all profile files so the `/apply` workflow works out of the box.
...
**Skip `documents/templates/` entirely.** It holds blank starters full of placeholder text, not the user's material. Never list it, read it, or extract from it.

If every subfolder is empty, stop and tell the user to populate the folder. Point them at `documents/templates/`, which holds a master CV, a LinkedIn profile and a professional-record starter to copy and fill in, and at `documents/README.md` for the layout.
...
- tailored CV markdown: summary statement, section ordering, framing for this role type
...
- **`03-writing-style.md`:** Source is archived tailored CV markdown and application-form field files. Extract recurring patterns. Add as observations under "## Patterns Observed in Past Applications". Do not modify existing rules. Only add if 2+ documents show a genuine pattern.
...
Then proceed to Step 3 to populate the non-skill files (`CLAUDE.md`, `documents/cv/master_cv.md`, `.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **Job portals:** "The framework ships market-agnostic search CLIs (`linkedin-search`, `freehire-search`) plus Polish portal skills (JustJoin.it, No Fluff Jobs, Czy jest eldorado). `/scrape` auto-discovers whatever portal skills are installed under `.agents/skills/`. Which of these fit your market, and do you use other job boards?" If the user needs a local board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
- **CV language:** "Should your CVs be written in English (the default, accepted in most markets), or in your market's language?" Record the answer as a `CV language: <language>` line in CLAUDE.md's Identity section. This setting governs the CV; form fields and portal messages follow each posting's language. If the user is unsure, keep English and note they can re-run `/setup --section search` to change it.
...
### 6. Update `06-interview-prep.md` *(Path B and C; skip if Path A populated it)*
...
### 7. Update `documents/cv/master_cv.md`
...
If Path A left any STAR stubs in `06-interview-prep.md`, also note:

> Path A flagged [N] STAR candidate stubs in `06-interview-prep.md` that need your situation/task/action/result details before you use them in interviews.
