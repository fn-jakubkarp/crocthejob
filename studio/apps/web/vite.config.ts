...
import { runApi } from "./server/run-api.ts";

// `jobsApi`, `runApi` and `filesApi` are dev-server plugins, not standalone backends,
// so `bun run dev` is the only process to run.
export default defineConfig({
	plugins: [react(), tailwindcss(), jobsApi(), runApi(), filesApi()],
